	<div id="sidebar-wrapper" style="background-color: #3C3C3C !important;">
		<ul class="nav nav-list">
			<li class="active"><a href="<?php echo base_url(); ?>"><i class="icon-dashboard icon-2x"></i> Dashboard </a></li> 
			<li><a href="sales.html"><i class="icon-shopping-cart icon-2x"></i> Sales</a>  </li>             
			<li><a href="<?php echo base_url();?>product_control"><i class="icon-list-alt icon-2x"></i> Products</a></li>
			<li><a href="<?php echo base_url();?>customer_control"><i class="icon-group icon-2x"></i> Customers</a></li>
			<li><a href="<?php echo base_url();?>Supplier_ctrl"><i class="icon-group icon-2x"></i> Suppliers</a></li>
			<li><a href="salesreport.html?d1=0&d2=0"><i class="icon-bar-chart icon-2x"></i> Sales Report</a></li>
			<li><a href="sales_inventory.html"><i class="icon-table icon-2x"></i> Product Inventory</a></li>
			<br><br><br><br><br><br>		
			<li>
			<div class="hero-unit-clock">
				<form name="clock">
					<font color="white">Time: <br></font>&nbsp;<input style="width:150px;" type="submit" class="trans" name="face" value="">
				</form>
			</div>
			</li>
		</ul>
	</div><!--/sidebar-wrapper-->